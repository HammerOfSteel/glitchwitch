"""One-off Blender script: merge the real Meshy AI Wren character mesh GLB
with its Meshy AI animation-library GLB into a single Wren body GLB with
named clips Godot's WrenAvatar loader expects (idle/walk/run).

Unlike the Seren placeholder's animations-only export (which only carried
Walking/Running plus a single baked base pose), this Meshy AI export ships a
generic ~20-clip animation library (idle/walk/run plus combat/swim/sleep
clips from a stock Meshy library). Glitch Witch has no combat, so only the
clips relevant to Wren's overworld locomotion are kept; the rest are
dropped so they don't ship in the GLB or show up as confusing options in
the AnimationPlayer.

Run headless: blender --background --python merge_wren_meshy.py -- <char.glb> <anims.glb> <out.glb>
"""
import sys
import bpy
...
print(f"Wrote merged Wren GLB to {out_path}")
